...
final class FolderCell: UICollectionViewCell {
    ...
    func update(by folder: CategoryDetailFolder.FolderList) {
        prepareSurfingFolder()
        titleLabel.text = folder.name
        countLabel.text = folder.linkCount.toAbbreviationString
        likeCountLabel.text = folder.likeCount.toAbbreviationString
    ...
    func update(by folder: UsersFolder.Result) {
        prepareSurfingFolder()
        titleLabel.text = folder.name
        countLabel.text = folder.linkCount.toAbbreviationString
        likeCountLabel.text = folder.likeCount.toAbbreviationString
        ...
        if let url = URL(string: folder.linkImageURL), folder.linkImageURL != "-1" {
            ...
            webPreviewImageView.kf.setImage(with: URL(string: Constant.defaultImageURL))
        }
    }
