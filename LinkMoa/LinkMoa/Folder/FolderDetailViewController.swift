...
final class FolderDetailViewController: UIViewController, CustomAlert, BackgroundBlur {
    ...
    var folderRemoveHandler: (() -> Void)? // Folder 삭제됬을 때 FolderVC 에서 삭제 Alert 올릴 때 사용
    var folderIndex: Int?
    ...
    @objc private func folderEditButtonTapped() {
        ...
        editVC.handlers = [ { [weak self] _ in // 폴더 수정 클릭
            ...
            self.presentTextRequestView(name: folderName) { _ in
                self.folderDetailViewModel.removeFolder(folderIndex: folderIndex) { result in
                    switch result {
    ...
    @objc private func cellEditButtonTapped(_ sender: UIGestureRecognizer) { // edit 버튼 클릭됬을 때
        ...
        editVC.handlers = [ { [weak self] _ in // 링크 수정
            ...
            self.folderDetailViewModel.deleteLink(link: link.index) { result in
                switch result {
...
extension FolderDetailViewController: UICollectionViewDelegate {
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        if let isUsingCustomBrowser = tokenManager.isUsingCustomBrowser, isUsingCustomBrowser {
            // 웹뷰로 링크열기
            webVC.url = url
